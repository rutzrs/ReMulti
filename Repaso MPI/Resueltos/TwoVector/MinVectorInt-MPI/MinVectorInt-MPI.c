...
void MinVectorInt(int *pVector1, int *pVector2, int * pVector3,
                   const int Rows)
{
 for (int i=0;i<Rows;i++)
...
{
 int Rows;
 int * pVector3=NULL, * pSubVector1=NULL, * pSubVector2=NULL, * pSubVector3=NULL;
 ...
 if (MyRank!=Root)
    {
     ...
     for (int i=0;i<pSizeSubV[MyRank];i++)
         pSubVector3[i]=(pSubVector1[i]<pSubVector2[i])?pSubVector1[i]:pSubVector2[i];
 ...
 if (MyRank==Root)
    {
     ...
     for (int i=pPosSubV[Root];i<pPosSubV[Root]+pSizeSubV[Root];i++)
         pVector3[i]=(pSubVector1[i-pPosSubV[Root]]<pSubVector2[i-pPosSubV[Root]])?pSubVector1[i-pPosSubV[Root]]:pSubVector2[i-pPosSubV[Root]];
