 ...
 if (MyRank!=Root)
    {
     ...
     MPI_Reduce(&LocalResta, &Resta, 1, MPI_INT, MPI_SUM, Root, MPI_COMM_WORLD);
    }
 ...
 if (MyRank==Root)
    {
     ...
     Resta = 0;
     for (int i=pPosSubV[Root];i<pSizeSubV[MyRank]+pPosSubV[Root];i++)
     	Resta-=pVector[i];
     ...
     MPI_Reduce(MPI_IN_PLACE, &Resta, 1, MPI_INT, MPI_SUM, Root, MPI_COMM_WORLD);
    }
