 ...
 if (MyRank!=Root)
    {
     ...
     for (int i=0;i<pSizeSubV[MyRank];i++)
        if (pSubVector[i] > LocalMax) LocalMax = pSubVector[i];
 ...
 if (MyRank==Root)
    {
     ...
     int Max=pVector[0];
     for (int i=0;i<pPosSubV[Root]+pSizeSubV[MyRank];i++)
        if (pVector[i] > Max) Max = pVector[i];
     ...
     MPI_Gatherv(MPI_IN_PLACE, pSizeSubV[MyRank], MPI_DATATYPE_NULL,
                 pVector,    pSizeSubV, pPosSubV, MPI_INT,
                 Root, MPI_COMM_WORLD);
