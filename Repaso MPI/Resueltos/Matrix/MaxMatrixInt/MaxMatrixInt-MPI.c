...
/*---------------------------------------------------------------------------*/
void InitMatrixMax(int *pMatrix, const int Size)
{
...
/*---------------------------------------------------------------------------*/
void PrintMatrixMax(int *pMatrix, const int Size, const int Cols)
{
 puts("-----------------------------------------");
 for (int i=0;i<Size;i++) {
	printf("%d,", pMatrix[i]);
...
int main(int argc, char **argv)
{
 ...
 if (MyRank!=Root)
    {
     ...
     InitMatrixMax (pSubMatrix,pSizeSubV[MyRank]);
     
     int LocalMax=-999;
     for (int i=0;i<pSizeSubV[MyRank];i++)
         if (pSubMatrix[i] > LocalMax) LocalMax = pSubMatrix[i];
 ...
 if (MyRank==Root)
    {
     ...
     InitMatrixMax (pMatrix,pSizeSubV[MyRank]);
 ...
 if (MyRank==Root)
    {
     #if (PRINT==1)
     PrintMatrixMax(pMatrix,Rows*Cols,Cols);
     #endif
     free((void *)pMatrix);
     printf("Max=%d.\n", Max);
    }
