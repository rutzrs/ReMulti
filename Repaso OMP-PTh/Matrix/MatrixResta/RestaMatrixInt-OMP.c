...
#ifdef OPENMP
#pragma omp parallel for default(none) private(i,j) shared(Rows,Cols,pMatrix) reduction(-:Resta) collapse(2)
#endif
 for (int i=0;i<Rows;i++)
    for (int j=0;j<Cols;j++)
      Resta -= pMatrix[i][j];
...
int main(int argc, char **argv)
{
 ...
 #if (DEBUG==1)
 printf("RestaMatrixInt-OMP: Rows=%d\tCols=%d.\n", Rows, Cols);
 #endif
 ...
 RestaMatrixInt (pMatrix,Rows,Cols, &Resta);
