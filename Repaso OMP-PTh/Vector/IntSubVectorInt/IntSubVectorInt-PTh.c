...
/*---------------------------------------------------------------------------*/
//Subs x to Vector
void * IntSubVectorInt(void * pTh_Args)
{
 Type_Th_SubV_Args * pMyData;
 pMyData=(Type_Th_SubV_Args *) pTh_Args;
 
 int rows_start, rows_end, dato;
 int * pVector;
...
int main(int argc, char **argv)
{
 ...
 printf("IntSubVectorInt-PTh: Rows=%d, NThreads=%d \n", Rows, NThreads);
 ...
 pThreads = (pthread_t *) GetMem(NThreads, sizeof(pthread_t), 
                                 "Main: pThreads");
 pTh_Args = (Type_Th_SubV_Args *) GetMem(NThreads, 
                                            sizeof(Type_Th_SubV_Args),
                                            "Main: pTh_Data");
 ...
 PrintVectorInt(pVector,Rows,"Vector");
  ...
  for (int t=0; t<NThreads; t++) 
	...
	rc = pthread_create(&pThreads[t],NULL, 
                          IntSubVectorInt, (void *)&pTh_Args[t]);
     }
